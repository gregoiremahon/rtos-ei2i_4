...
int main( void )
{
 // Configuration minimal du processeur au niveau périphériques
 //pour le projet
	prvSetupHardware();
	/* initialisation des taches */
	vInit_myTasks( mainLED_TASK_PRIORITY );
	// toutes les taches ont été créées - Demarrer le scheduler.
  // Les taches tournent en USER/SYSTEM mode  :  USER UNPRIVILEGED
	//le Scheduler tourne en Superviseur mode   :  HANDLER 
	// Le processeur doit basculer  quand vTaskStartScheduler est appelé
  // cela est possible car le processeur démarre en user privileged 
	vTaskStartScheduler();
