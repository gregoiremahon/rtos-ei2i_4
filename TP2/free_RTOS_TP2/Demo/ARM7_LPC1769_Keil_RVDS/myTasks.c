...
const uint32_t sequence_moving[ANIMATION_LENGTH_MOVING] = {
    // S�quence pour le v�hicule en mouvement
    1 << 7, 1 << 6, 1 << 5, 1 << 4, 
    1 << 3, 1 << 4, 1 << 5, 1 << 6
};

const uint32_t sequence_freezed[ANIMATION_LENGTH_FREEZED] = {
    // S�quence pour le v�hicule � l'arr�t
...
void initChenillard() {
    LPC_GPIO2->FIODIR |= 0xF8; // Configure P2.3 � P2.7 en sortie
}

void initCodeur() {
    LPC_GPIO2->FIODIR &= ~((1 << 11) | (1 << 12) | (1 << 10)); // Configurer P2.11, P2.12 et P2.10 comme entr�es
}
...
void init_dac() {
    ...
    LPC_DAC->DACR = 0x00007FC0; // DAC initialise � 1.65 Volt
}
...
void init_adc() {
    ...
    LPC_PINCON->PINSEL1 |= (0x01 << 14) | (0x01 << 16) | (0x01 << 18); // 3 entr�es analogiques
    LPC_PINCON->PINMODE1 |= (0x02 << 14) | (0x02 << 16) | (0x02 << 18); // 3 entr�es sans pull-up ni pull-down
    LPC_ADC->ADCR |= 1 << 8; // Horloge de l'ADC � 12.5 MHz
    LPC_ADC->ADINTEN = 0x04; // AD0.2 source de l'IT de fin de conversion
    LPC_ADC->ADCR |= 0x7; // Lancer les conversions AD0.0, AD0.1, AD0.2
    NVIC_SetPriority(ADC_IRQn, 5); // Choisir une priorit� compatible avec l'OS
...
void init_PWM() {
    ...
    LPC_PINCON->PINSEL4 |= 0x00000005; // P2.0 � P2.1 deviennent PWM1.1, PWM1.2
    LPC_PWM1->TCR = 0x03;
    LPC_PWM1->PCR |= (0x03 << 9); // Deux PWM activ�s
    LPC_PWM1->MR0 = 4094; 
    LPC_PWM1->MR1 = 2048;
    LPC_PWM1->MR2 = 2048;
    LPC_PWM1->LER = 0x07;   
...
void ADC_IRQHandler(void) {
    LPC_ADC->ADCR &= ~(1 << 16); // Arr�t de la conversion
    val_ADC0 = (LPC_ADC->ADDR0 >> 6) & 0x3FF; // R�cup�ration des valeurs converties
...
void adjustMotorSpeed(uint32_t speedMotor1, uint32_t speedMotor2) {
    ...
    LPC_PWM1->LER |= 0x03; // Mise � jour des registres
}

void toggleLEDsInSequence() {
	  // Fonction pour le chenillard, allume les LEDs en s�quence.
    static uint32_t ledState = 0;

    LPC_GPIO2->FIOCLR = 0x0F8; // �teindre toutes les LEDs
    LPC_GPIO2->FIOSET = (1 << (3 + ledState)); // Allumer une LED

    ledState = (ledState + 1) % 5; // Passer � la LED suivante
}
...
void maj_dacr() {
    LPC_DAC->DACR = val_son; // Mise � jour du DAC
...
unsigned char readButton1() {
    // lecture de l'�tat du bouton sur P0.0
    return (LPC_GPIO0->FIOPIN0 & 1) ? BP_OFF : BP_ON;
}

unsigned char readButton2() {
    // lecture de l'�tat du bouton sur P0.1
    return (LPC_GPIO0->FIOPIN0 & 2) ? BP_OFF : BP_ON;
}

unsigned char lectureEtatCodeur() {
    return (LPC_GPIO2->FIOPIN >> 11) & 0x03; // Lecture des �tats des pins du codeur
}
...
static void vSoundTask(void *pvParameters) {
    ...
    init_dac();
    xLastWakeTime = xTaskGetTickCount(); // Initialiser la derni�re heure de r�veil

    while (1) {
        maj_dacr();

        // V�rifier si le temps �coul� depuis le dernier calcul est suffisant
        if (xTaskGetTickCount() - xLastWakeTime > xFrequency) {
            calcul_tableau(tableau_son);
            xLastWakeTime = xTaskGetTickCount(); // R�initialiser la derni�re heure de r�veil
        }
...
static void vChenillardTask(void *pvParameters) {
    ...
    while (1) {
        LPC_GPIO2->FIOCLR = 0xF8; // �teindre toutes les LEDs avant de changer l'�tat
...
static void vMotorControlTask(void *pvParameters) {
    ...
    // Initialisation du PWM pour le contr�le des moteurs
    init_PWM();
    ...
    while (1) {
        ...
        // Appliquer les vitesses ajust�es aux moteurs
...
static void vSensorTask(void *pvParameters) {
    ...
    while (1) {
        codeur = lectureEtatCodeur(); // Lire l'�tat actuel du codeur
        ...
        // Limiter la vitesse diff�rentielle � VITESSE_MAX
        if (differentiel_vitesse > VITESSE_MAX) differentiel_vitesse = VITESSE_MAX;
        if (differentiel_vitesse < -VITESSE_MAX) differentiel_vitesse = -VITESSE_MAX;

        old_codeur = codeur;

        // Notifier la t�che de mise � jour des PWM si n�cessaire
        if (old_codeur != codeur) {
            // Code pour notifier la t�che PWM (� impl�menter)
        }

        vTaskDelay(pdMS_TO_TICKS(5)); // Fr�quence de 200 Hz
    }
}
